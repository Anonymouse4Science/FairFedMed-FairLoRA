...
class FairLoRALinear(nn.Module):
    ...
    def reset_parameters(self):
        ...
        init_type = 'cycle_shift' # 'same+cycle'
...
class GLP_OT_SVLoRA(TrainerX):
    ...
    def forward_backward(self, batch, is_last_client=False):
        ...
        if prec == "amp":
            ...
            #     unique_attr_labels, counts = torch.unique(attr, return_counts=True)

            #     # Initialize the weights tensor with zeros
            #     weights = torch.zeros_like(attr, dtype=loss.dtype).to(loss.device)

            #     # Assign weights based on the inverse of the counts and normalize
            #     for attr_label, count in zip(unique_attr_labels, counts):
            #         label_weight = (1/len(unique_attr_labels)) / count.item()  # Normalize so the total weight per class is 1/3
            #         weights[attr == attr_label] = label_weight
