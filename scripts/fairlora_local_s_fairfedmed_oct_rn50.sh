...
FRAC=0.7
ROUND=50
STEPSIZE=240  # 2 * ROUND * 0.5 * max(USERS * FRAC, 1)
NUM_PROMPT=2
# DATASET=$1
...
do
  ...
  do
    for SEED in 1
    do
      DIR=output/FairLoRA_${CFG}_oct/${DATASET}_${MODALITY_TYPE}_${PARTITION}_beta${BETA}_${LoRA_TYPE}_rank${LoRA_RANK}_alpha${LoRA_ALPHA}_local_s_itv4_slice${DIM_PER_3D_SLICE}_norm/${MODEL}_${TRAINER}_${OT}_${TOP_PERCENT}_eps${EPS}_${ATTRIBUTE_TYPE}/nctx${NCTX}_csc${CSC}_ctp${CTP}/iid_${IID}_${USERS}users_${FRAC}frac_lr${LR}_${ROUND}round_seed${SEED}
      if [ -d "$DIR" ]; then
